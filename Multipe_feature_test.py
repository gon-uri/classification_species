...
- Mean, width and entropy of Syllabe Duration
...
for i in range(len(files)):
    ...
    min_index = np.argmin(
        returns_dist_smooth_norm[non_zero_init:non_zero_end]) + non_zero_init
    min_value = returns_dist_smooth_norm[min_index]
    # Gets indexs greater half the size of the maximun
    high_indexs = np.nonzero(
        returns_dist_smooth_norm[0:-1] > (min_value + (max_value - min_value) / 2))[0]
    ...
    for n in range(0, len(limits), 2):
        ...
        # inter_max_index = limits[n]+int((limits[n+1]-limits[n])/2)
        if (limits[n] != limits[n + 1]):
            inter_max_index = np.argmax(
                returns_dist_smooth_norm[limits[n]:limits[n + 1]])
            inter_max_index_list.append(inter_max_index)
            inter_width_list.append(
                temp_window[limits[n + 1]] - temp_window[limits[n]])
            inter_max_value_list.append(
                returns_dist_smooth_norm[inter_max_index])
...
labels = ['Subject 0', 'Subject 1', 'Subject 2', 'Subject 3', 'Subject 4',
...
for j in range(len(bird_id_list)):
    if old_id == bird_id_list[j]:
        ...
        ax1.plot(freq_list[j], fourier_list[j],
                 colors[bird_id_list[j]], label=labels[bird_id_list[j]])
...
for j in range(len(bird_id_list)):
    if old_id == bird_id_list[j]:
        ax2.plot(times_list[j][0:len(cr_distribution_list[j])],
                 cr_distribution_list[j], colors[bird_id_list[j]])
    else:
        ax2.plot(times_list[j][0:len(cr_distribution_list[j])], cr_distribution_list[j],
                 colors[bird_id_list[j]], label=labels[bird_id_list[j]])
    old_id = bird_id_list[j]
...
for j in range(len(bird_id_list)):
    if old_id == bird_id_list[j]:
        ax3.scatter(fou_max_list[j], fou_wid_list[j],
                    c=colors[bird_id_list[j]], s=200)
    else:
        ax3.scatter(fou_max_list[j], fou_wid_list[j],
                    c=colors[bird_id_list[j]], s=200, label=labels[bird_id_list[j]])
    ax3.annotate(str(bird_id_list[j]), (fou_max_list[j], fou_wid_list[j]))
    old_id = bird_id_list[j]
ax3.set(xlabel='Max Position (Hz)',
        ylabel='Width (Hz)', title='Fourier Maximum')
...
for j in range(len(bird_id_list)):
    if old_id == bird_id_list[j]:
        ax4.scatter(cr_max_list[j], cr_wid_list[j],
                    c=colors[bird_id_list[j]], s=200)
    else:
        ax4.scatter(cr_max_list[j], cr_wid_list[j],
                    c=colors[bird_id_list[j]], s=200, label=labels[bird_id_list[j]])
    ...
    old_id = bird_id_list[j]
ax4.set(xlabel='Max Position (Seconds)',
        ylabel='Width (Seconds)', title='Close Returns Maximum')
plt.legend(prop={'size': 15})
plt.draw()
...
for j in range(len(bird_id_list)):
    if old_id == bird_id_list[j]:
        ax5.scatter(
            fou_entropy_list[j], cr_entropy_list[j], c=colors[bird_id_list[j]], s=200)
    else:
        ax5.scatter(fou_entropy_list[j], cr_entropy_list[j],
                    c=colors[bird_id_list[j]], s=200, label=labels[bird_id_list[j]])
    ax5.annotate(str(bird_id_list[j]),
                 (fou_entropy_list[j], cr_entropy_list[j]))
    old_id = bird_id_list[j]
